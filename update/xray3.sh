...
#
#
xrays1=$(systemctl restart xray | grep Active: | awk '{print $2}')
...
#
if [ "$xrays" == "$ell" ]; then
 echo -e " Xray/V2Ray              :$GREEN [Restarted Succesfully] $NC"
 echo -e " Xray/V2Ray              :$GREEN [Running] $NC"
 else
  echo -e " Xray/V2Ray              :$GREEN [Restarted unsuccess repair xray plz.] $NC"
